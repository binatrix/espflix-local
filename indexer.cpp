...
void make_video(const string& src, const string& dst, bool clean = false)
{
    ...
    const char* video =  "%s -y -loglevel verbose -i %s/video.mp4 -s 352x192 -f mpegts -pat_period 30 -streamid 1:258 -pes_payload_size 512 -codec:v mpeg1video -qmin 3 -qmax 1000 -b:v 1500k -maxrate 1500k -bufsize 0.25M -bf 0 -codec:a sbc -ar 48000 -b:a 192k -ac 1 -filter:v fps=fps=24 -max_muxing_queue_size 1024 %s/video.ts";
...
{
    if (argc != 3) {
        printf("Missing params. You must provide the source and the target folders\n");
        printf("Example:\n");
        printf("indexer ./mp4 /var/www/html/espflix/service\n");
        return -1;
    }
    printf("Converting videos from \"%s\" to \"%s\"\n", argv[1], argv[2]);
    make_service(argv[1], argv[2]);
    printf("Process end\n");
    return 0;
}